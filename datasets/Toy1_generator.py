...
# -*- coding: utf-8 -*-
"""
Created on Tue Mar 27 20:29:37 2018
...
num_gains = int(dataset_size / num_harm)  # Caused number of gains for each harmonic
...
if PHASE == True:
    Spectrums = np.zeros([nb_bins* 2, dataset_size])  # Output 1
else:
    Spectrums = np.zeros([nb_bins, dataset_size])
...
modules_temp = modules_temp[0:nb_bins]

if PHASE == True:
    phases_temp = np.angle(spectrum_temp)
    Spectrums[nb_bins:, 0] = phases_temp[0:nb_bins]

Spectrums[:nb_bins, 0] = modules_temp
...
for harmonic in range(2, num_harm + 1):
    ...
    for harmonic_gain in range(1, num_gains + 1):
        ...
        if PHASE == True:
            phases_temp = np.angle(spectrum_temp)
            phases_temp = phases_temp[0:nb_bins]
            Spectrums[nb_bins:, inc] = phases_temp

        # Store in numpy array
        Spectrums[:nb_bins, inc] = modules_temp
...
if PHASE:
    phase_l = '-phase-'
    specPos[nb_bins:, :] = Spectrums[nb_bins:, :]

specPos[:, :nb_bins] = Spectrums[:, :nb_bins] - \
    np.min(Spectrums[:, :nb_bins])
...
# dB normalized
specDbNorm = specPos
specDbNorm[:, :nb_bins] = specPos[:, :nb_bins] / \
    np.max(specPos[:, :nb_bins])
...
specLin[:, :nb_bins] = 10**(Spectrums[:, :nb_bins] / 20)

if PHASE:
    specLin[:, :nb_bins] = Spectrums[:, :nb_bins] - \
        np.min(Spectrums[nb_bins:, :])
    specLin[:, nb_bins:] = specLin[:, nb_bins:] * \
        np.max(specLin[:, :nb_bins]) / np.max(specLin[:, nb_bins:])
...
# linear normalized
specLinNorm = specLin
specLinNorm[:, :nb_bins] = specLin[:, :nb_bins] / \
    np.max(specLin[:, :nb_bins])
specLinNorm[:, nb_bins:] = specLin[:, nb_bins:] - \
    np.min(specLin[:, nb_bins:])
specLinNorm[:, nb_bins:] = specLin[:, nb_bins:] / \
    np.max(specLin[:, nb_bins:])
